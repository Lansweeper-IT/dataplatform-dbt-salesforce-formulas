{%- macro sfdc_formula_refactor(join_to_table, source_name = 'PROD_STAGING_SALESFORCE_MODELPREP') -%}

    --Generate the key value pair from the formula field table with the below macro.
    {%- set key_val = sfdc_get_formula_column_values(source(source_name, 'FIVETRAN_FORMULA'), 'field', 'sql', join_to_table) -%}
